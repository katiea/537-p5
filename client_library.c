...
int MFS_Stat(int inum, MFS_Stat_t *m){
    ...
    if (rc > 0) {
	    ...
	    rc = UDP_Read(sd, &raddr, p, sizeof(struct __MFS_package_t));
    }
    
    memcpy(m, p->m, sizeof(MFS_Stat_t));
    
    if (p->success == 1) return 0;
    else return -1;
...
int MFS_Read(int inum, char *buffer, int block){
    ...
    p->int_arg1 = inum;
    p->int_arg2 = block;
    //make sure to put information into "d"
    p->success = 0;
    ...
    MFS_DirEnt_t *temp = malloc(sizeof(struct __MFS_DirEnt_t));
    memcpy(temp, p->d, sizeof(*(p->d)));
